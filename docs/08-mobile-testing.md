...
- **Native OAuth uses a server-side handoff, not a deep link.** Base44 rejects custom schemes as redirect domains, and returning to the app via `exp://` / `findmybins://` proved unreliable (in Expo Go the scheme reloads the whole project and the token is lost — this is why Google sign-in kept bouncing back to the login screen). Instead: the app generates an unguessable 32-char handoff id, opens the provider with the relay page (`public/native-auth.html?handoff=<id>`) as `from_url`; that static page stores the freshly issued token under the id via `api/auth-handoff store`; the app polls `api/auth-handoff claim` and picks it up. Records are single-use and expire in 2 minutes; the id is stored only as a hash. No custom-scheme round trip is involved. Covered by `tests/e2e/auth-handoff.mjs`.
...
- **Always start the dev server from the repo root via the npm scripts** (`npm run app`, or `npm run app:clear` to reset Metro's cache). Running `npx expo start` from the repo root directly points Metro at the root `package.json`, which has no `expo-router/entry` main, so it falls back to `expo/AppEntry` and fails with *Unable to resolve "../../App"*. `npm run app` runs Expo inside `apps/app`, which is correct.
