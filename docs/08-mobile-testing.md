...
- **Native OAuth uses a static relay page.** Base44 preserves `from_url` through the provider round trip, but **validates the redirect domain at its callback step and rejects custom schemes** ("Invalid redirect domain"), so the app can't be the redirect target directly. Native sign-in returns to `public/native-auth.html` on the https domain, which hands the token to `exp://…/--/auth-callback` (Expo Go) or `findmybins://auth-callback` (a build).
  That page is deliberately static — routing it through the SPA meant loading a 1.9 MB React bundle inside the auth sheet. It auto-attempts the scheme navigation and, because browsers often block scripted navigation to a custom scheme without a user gesture, falls back to a tappable button. It only ever forwards to this app's own schemes. Covered by `tests/e2e/relay-auth.mjs`.
  `app/auth-callback.tsx` remains the fallback for when the OS delivers the deep link to the app instead of resolving the auth session.
